...
//
// audio context still has some issues because the "import synth" statement
// already tries to use Tone.Js
//
async function checkFirstUserInteraction(){
    if(!userInteracted) {
        await Tone.start()
        synth = new Synth()
        userInteracted = true
...
async function keyPressed(note) {
    // todo better way to force a first user interaction
    await checkFirstUserInteraction() 
